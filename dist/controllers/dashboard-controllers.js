...
exports.verificationController = exports.fanDashboardController = exports.creatorDashboardController = exports.adminDashboardController = exports.BaseDashboardController = exports.VerificationController = exports.FanDashboardController = exports.CreatorDashboardController = exports.AdminDashboardController = void 0;
const logger_1 = require("../utils/logger");
const money_orchestrator_1 = require("../services/money-orchestrator");
const fanztrust_service_1 = require("../services/fanztrust-service");
// ========================================
// ADMIN DASHBOARD CONTROLLER
// ========================================
class AdminDashboardController {
    logger = new logger_1.Logger('AdminDashboardController');
    /**
    ...
    async getFinancialOverview(req, res) {
        try {
            ...
            const [revenueData, payoutData, transactionCounts, disputeData, trustScoreData, processorHealth] = await Promise.all([
                this.getRevenueData(timeframe, platforms),
                this.getPayoutData(timeframe),
     ...
     */
    async getSettlementReports(req, res) {
        try {
            const user = this.extractUser(req);
            this.validateAdminAccess(user);
            ...
            const settlement = await money_orchestrator_1.moneyOrchestrator.generateSettlementSummary(dateFrom, dateTo, processors ? processors.split(',') : undefined);
     ...
     */
    async processRefundApproval(req, res) {
        try {
            const user = this.extractUser(req);
            this.validateAdminAccess(user);
            ...
            // Get approval record
            const approval = await this.database.query('SELECT * FROM approvals WHERE id = $1 AND entity_type = $2', [approvalId, 'refund']);
            if (!approval[0]) {
            ...
            if (action === 'approve') {
                ...
                await money_orchestrator_1.moneyOrchestrator.processRefund(refundData.transactionId, refundData.reason, refundData.reasonDetails, refundData.evidence);
            }
    ...
    async getRevenueData(timeframe, platforms) {
        // Implementation to get revenue data
        return { total: '0.00' };
    }
    async getPayoutData(timeframe) {
        // Implementation to get payout data
        return { total: '0.00' };
    }
    async getTransactionCounts() {
        const result = await this.database.query("SELECT COUNT(*) as count FROM transactions WHERE status = 'pending'");
        return { pending: parseInt(result[0]?.count || '0') };
    }
    async getDisputeData() {
        const result = await this.database.query("SELECT COUNT(*) as count FROM disputes WHERE status IN ('open', 'fighting')");
        return { active: parseInt(result[0]?.count || '0') };
    }
    async getTrustScoreMetrics() {
        // Implementation to get trust score metrics
        return { average: 75, trending: 'stable' };
    }
    async getProcessorHealthMetrics() {
...
class CreatorDashboardController {
    logger = new logger_1.Logger('CreatorDashboardController');
    /**
    ...
    async getFinancialSummary(req, res) {
        try {
            ...
            const [earnings, balance, payoutInfo, metrics] = await Promise.all([
    ...
    async requestPayout(req, res) {
        try {
            ...
            const result = await money_orchestrator_1.moneyOrchestrator.processPayout(payoutRequest);
    ...
    async getCreatorPayoutInfo(creatorId) {
...
// ========================================
class FanDashboardController {
    logger = new logger_1.Logger('FanDashboardController');
    /**
    ...
    async processPayment(req, res) {
        try {
            ...
            const result = await money_orchestrator_1.moneyOrchestrator.processPayment(paymentRequest);
    ...
    async requestRefund(req, res) {
        try {
            ...
            const result = await money_orchestrator_1.moneyOrchestrator.processRefund(transactionId, reason, reasonDetails, evidence);
    ...
    async getFanSpendingData(fanId) {
...
exports.FanDashboardController = FanDashboardController;
// ========================================
// VERIFICATION CONTROLLER
// ========================================
class VerificationController {
    logger = new logger_1.Logger('VerificationController');
    /**
    ...
    async verifyTransaction(req, res) {
        try {
            ...
            const result = await fanztrust_service_1.fanzTrustEngine.verifyTransaction(verificationRequest);
...
class BaseDashboardController {
    logger = new logger_1.Logger('BaseDashboardController');
    database = null; // Would be injected
    extractUser(req) {
...
exports.creatorDashboardController = new CreatorDashboardController();
exports.fanDashboardController = new FanDashboardController();
